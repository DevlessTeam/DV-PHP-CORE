...
var _jql = $;
$.noConflict();
function assignLabel(){num=Math.round(100*Math.random())+1;var a=window.adjs,b=window.nouns;return adj=a[Math.floor(Math.random()*a.length)],noun=b[Math.floor(Math.random()*b.length)],separator="-",adj+separator+noun+separator+num}adjs="autumn hidden bitter misty silent empty dry dark summer icy delicate quiet white cool spring winter patient twilight dawn crimson wispy weathered blue billowing broken cold damp falling frosty green long late lingering bold little morning muddy old red rough still small sparkling throbbing shy wandering withered wild black young holy solitary fragrant aged snowy proud floral restless divine polished ancient purple lively nameless".split(" "),nouns="waterfall river breeze moon rain wind sea morning snow lake sunset pine shadow leaf dawn glitter forest hill cloud meadow sun glade bird brook butterfly bush dew dust field fire flower firefly feather grass haze mountain night pond darkness snowflake silence sound sky shape surf thunder violet water wildflower wave water resonance sun wood dream cherry tree fog frost voice paper frog smoke star".split(" ");

(function(l){function h(c,a,b,d,e){e=e||!1;var f=new XMLHttpRequest;f.addEventListener("readystatechange",function(){var a="";4===this.readyState&&0==e?200==this.status?(a=JSON.parse(this.responseText),d(a)):(d(a),console.error("Devless cannot be found at "+this.devless_instance_url+" Please copy the url from the `App tab`  on you Devless instance by clicking on  `connect to my app`")):4===this.readyState&&1==e&&(200==this.status?(a=this.responseText,d(a)):(d(a),console.error("Devless cannot be found at "+
this.devless_instance_url+" Please copy the url from the `App tab`  on you Devless instance by clicking on  `connect to my app`")))});f.open(b.toUpperCase(),this.devless_instance_url+a);f.setRequestHeader("content-type","application/json");f.setRequestHeader("devless-token",this.devless_token);""!=localStorage.getItem("devless_user_token"+this.devless_instance_url+this.devless_token)&&f.setRequestHeader("devless-user-token",localStorage.getItem("devless_user_token"+this.devless_instance_url+this.devless_token));
f.send(c)}var k=function(c){if(c){console.info("App is trying to connect to Devless ...");var a=new k.init(c);l.returnedInstance="";h.call(a,{},"/api/v1/service//dvauth/script","POST",function(b){b=JSON.parse(b);631==b.status_code?console.error("Your app failed to  connect to Devless ): Please make sure token and key is set properly "):(1E3==b.status_code&&console.debug("Your app connected to Devless successfully and you have auth service installed"),returnedInstance=a)},!0);return a}console.error("Your app failed to  connect to Devless ): Please make sure token and key is set properly ")};
k.prototype={queryData:function(c,a,b,d){b=b||{};var e="",f=function(a,b){for(var c in b)e="&"+a+"="+b[c]+e},g;for(g in b)b.hasOwnProperty(g),b[g]instanceof Array?f(g,b[g]):e="&"+g+"="+b[g]+e;h.call(this,"","/api/v1/service/"+c+"/db?table="+a+e,"GET",function(a){d(a)});return this},addData:function(c,a,b,d){a=JSON.stringify({resource:[{name:a,field:[b]}]});h.call(this,a,"/api/v1/service/"+c+"/db","POST",function(a){d(a)});return this},updateData:function(c,a,b,d,e,f){a=JSON.stringify({resource:[{name:a,
params:[{where:b+","+d,data:[e]}]}]});h.call(this,a,"/api/v1/service/"+c+"/db","PATCH",function(a){f(a)});return this},deleteData:function(c,a,b,d,e){a={resource:[{name:a,params:[{where:b+",=,"+d}]}]};a.resource[0].params[0]["delete"]="true";a=JSON.stringify(a);h.call(this,a,"/api/v1/service/"+c+"/db","DELETE",function(a){e(a)});return this},getToken:function(c){if(c)c(localStorage.getItem("devless_user_token"+this.devless_instance_url+this.devless_token));else return localStorage.getItem("devless_user_token"+
this.devless_instance_url+this.devless_token)},setToken:function(c){localStorage.setItem("devless_user_token"+this.devless_instance_url+this.devless_token,c);return!0},call:function(c,a,b,d){b=JSON.stringify({jsonrpc:"2.0",method:c,id:Math.floor(1E7*Math.random())+1,params:b});h.call(this,b,"/api/v1/service/"+c+"/rpc?action="+a,"POST",function(a){d(a)})}};k.init=function(c){this.devless_token=c.token;this.devless_instance_url=c.domain};k.init.prototype=k.prototype;l.Devless=l.DV=k})(window);
...
devless_main.coreLib.paginate = function(component, properties) {
	...
	var pager = _jql(reference).find('.dv-pager')[0]
	if(!pager) {return reference;}
	
	paginateTemplate = pager.cloneNode(true, true);
...
scriptEngine.pager = function(direction) {return this;}
...
scriptEngine.oneto = function(service, table) {
	persist = function(storeData, callback) {
		...
		if(window.event != undefined){
			label = window.event.target.label
		 }else {
		 	label = "temp name"; 
		 }
