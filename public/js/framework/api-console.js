function init() {
  console.log("API Console rendered");
}
  ...
  (function() {
    ...
    $("#service").change(function() {
      ...
      $.get("console/" + service_id, function(data) {
        ...
        for (var i = 0; i < table.length; i++) {
          $("#table").append(
            "<option>" + $.parseJSON(table[i].schema).name + "</option>"
          );
        }
      });
    });
    ...
    $("#operation").change(function() {
      ...
      if (request_type == "retrieve_all") {
        ...
        $.get(
          "/console/" + service_id + "/" + service_name + "/" + table_name,
          function(data) {
            console.log(data);
            var schema = data;
            var values = {};
            ...
            if (request_type === "create") {
              ...
              var json = JSON.stringify(
                JSON.parse('[{"where":"id, ","data":[{"key":"value"}]}]'),
                null,
                4
              );
            } else if (request_type === "delete") {
              var json = JSON.stringify(
                JSON.parse('[{"delete":"true","where":"id, "}]'),
                null,
                4
              );
            }
    ...
    $("#form_data").submit(function(e) {
      ...
      switch (request_type) {
        case "retrieve_all":
          ...
          query_url =
            "api/v1/service/" + service_name + "/db?table=" + table_name;
